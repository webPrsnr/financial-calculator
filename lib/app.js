class Model {
  ...
  deleteTransaction(id) {
    ...
    localStorage.clear();
    transactions
      .filter((transaction) => transaction.id !== parseInt(id))
      .map((el) => this.setLocalStorageitems(el));
    this.calculateExpense();
    this.transactionsChanged(
      this.getLocalStorageItems(),
      this.balance,
...
class View {
  ...
  bindAdd(handler) {
    this.form.addEventListener("submit", (event) => {
      ...
      if (this.text.value === "" || this.amount.value === "") {
...
class Controller {
  constructor(model, view) {
    ...
    this.view.deleteTransaction(this.handleDelete);

    this.transactionsChanged(
      this.model.getLocalStorageItems(),
      this.model.balance,
      this.model.income,
      this.model.expense
    );
